...
Most Wayland compositors and desktop environments support setting custom hotkeys that automatically provide activation tokens. Configure your hotkey to run:
```bash
/path/to/khala
```
...
**For manual activation (advanced):**
XDG activation tokens are opaque strings generated by the Wayland compositor to prevent focus stealing while allowing legitimate window activation. In most cases, tokens are automatically provided when launching applications through desktop environment hotkeys.

If you need to generate a token manually (rare), you would typically:
1. Request a token from another Wayland application that has focus
2. Use the compositor's IPC if available
3. Set the token as an environment variable:

```bash
# Example with a hypothetical token (actual tokens are compositor-generated)
export XDG_ACTIVATION_TOKEN="compositor-generated-token-string-here"
/path/to/khala
unset XDG_ACTIVATION_TOKEN  # Clean up after use
```

**Note:** Tokens are implementation-specific strings generated by your compositor. They cannot be manually created and must come from legitimate user interactions or compositor APIs.
