                          ...
                          uint32_t key, uint32_t state)
{
    ...
    const xkb_keysym_t keysym =
        xkb_state_key_get_one_sym(win->kb_state, keycode);
    const ui::KeyModifier modifiers = get_xkb_modifiers(win->kb_state);
    ...
    default:
        ...
        if (keysym >= XKB_KEY_a && keysym <= XKB_KEY_z &&
            ...
            event.key = static_cast<ui::KeyCode>(
                static_cast<int>(ui::KeyCode::A) + (keysym - XKB_KEY_a));
            win->pending_events.push_back(event);
            break;
        }
        ...
        if (keysym >= XKB_KEY_0 && keysym <= XKB_KEY_9 &&
            ui::has_modifier(modifiers, ui::KeyModifier::Ctrl)) {
            event.key = static_cast<ui::KeyCode>(
                static_cast<int>(ui::KeyCode::Num0) + (keysym - XKB_KEY_0));
            win->pending_events.push_back(event);
            break;
        }
        ...
        if (size > 0 && size < static_cast<int>(sizeof(buffer))) {
            ...
            if (buffer[0] >= 32 && buffer[0] < 127 && buffer[1] == '\0') {
                ...
                // Clear modifiers for regular character input
...
void pointer_enter_handler(void *data, wl_pointer *, uint32_t serial,
                           wl_surface *, wl_fixed_t surface_x,
                           wl_fixed_t surface_y)
{
    ...
    win->last_pointer_position = ui::WindowCoord{
        .x = wl_fixed_to_int(surface_x), .y = wl_fixed_to_int(surface_y)};

    win->pending_events.push_back(
        ui::CursorEnterEvent{.position = win->last_pointer_position});
}
...
void pointer_motion_handler(void *data, wl_pointer *, uint32_t,
                            wl_fixed_t surface_x, wl_fixed_t surface_y)
{
    ...
    win->pending_events.push_back(
        ui::MousePositionEvent{.position = win->last_pointer_position});
}

void pointer_button_handler(void *data, wl_pointer *, uint32_t, uint32_t,
                          ...
                          wl_fixed_t value)
{
    ...
    win->pending_events.push_back(ui::MouseScrollEvent{
        .direction = direction, .position = win->last_pointer_position});
}

static void pointer_frame_handler(void *, wl_pointer *) {}
static void pointer_axis_source_handler(void *, wl_pointer *, uint32_t) {}
static void pointer_axis_stop_handler(void *, wl_pointer *, uint32_t, uint32_t)
{
}
...
static void buffer_release_handler(void *data, wl_buffer *)
{
    ...
    LOG_DEBUG("Buffer released: %dx%d", buffer_entry->width,
              buffer_entry->height);
    buffer_entry->state = WaylandBuffer::State::FREE;
}

static const wl_buffer_listener buffer_listener = {.release =
                                                       buffer_release_handler};

// PlatformWindow implementation

PlatformWindow::PlatformWindow(ui::RelScreenCoord, ui::RelScreenCoord dimension)
    : display(nullptr), compositor(nullptr), surface(nullptr), seat(nullptr),
      ...
      shm(nullptr), current_buffer(nullptr), frame_counter(0), width(0),
      height(0), screen_width(1920), screen_height(1080)
{
    ...
    xdg_toplevel_set_min_size(toplevel, static_cast<int>(width),
                              static_cast<int>(height));
    xdg_toplevel_set_max_size(toplevel, static_cast<int>(width),
                              static_cast<int>(height));
...
void PlatformWindow::create_wl_buffer(WaylandBuffer &buf, unsigned int w,
                                      unsigned int h)
{
    ...
    wl_shm_pool *pool =
        wl_shm_create_pool(shm, buf.shm_fd, static_cast<int32_t>(buffer_size));
    if (!pool) {
    ...
    buf.wl_buffer_obj = wl_shm_pool_create_buffer(
        pool, 0, static_cast<int32_t>(w), static_cast<int32_t>(h),
    ...
    buf.cairo_surface = cairo_image_surface_create_for_data(
        static_cast<unsigned char *>(buf.shm_data), CAIRO_FORMAT_ARGB32,
...
WaylandBuffer *PlatformWindow::find_free_buffer(unsigned int buf_width,
                                                unsigned int buf_height)
{
    for (auto &buf : buffer_pool) {
        if (buf.state == WaylandBuffer::State::FREE && buf.width == buf_width &&
...
void PlatformWindow::cleanup_old_buffers()
{
    ...
    if (!candidates.empty()) {
        ...
        // Remove from pool
        buffer_pool.erase(std::remove_if(buffer_pool.begin(), buffer_pool.end(),
                                         [&](WaylandBuffer &b) {
                                             return &b == candidates[0];
                                         }),
                          buffer_pool.end());
        LOG_DEBUG("Cleaned up old buffer, pool size now: %zu",
                  buffer_pool.size());
    }
}

WaylandBuffer *PlatformWindow::allocate_buffer(unsigned int buf_width,
                                               unsigned int buf_height)
{
    ...
    // Step 3: Pool is full - try to destroy oldest FREE buffer with wrong
    // dimensions
    cleanup_old_buffers();
...
bool PlatformWindow::surface_cache_valid() const
{
    ...
    if (current_buffer->state != WaylandBuffer::State::DRAWING) {
        LOG_DEBUG("Surface cache miss: current_buffer->state != "
                  "WaylandBuffer::State::DRAWING");
        return false;
    }
...
void PlatformWindow::commit_surface()
{
    ...
    if (current_buffer == nullptr || surface == nullptr) {
        throw std::runtime_error(
            "Cannot commit surface: missing buffer or surface");
    }
    ...
    wl_surface_damage_buffer(surface, 0, 0, static_cast<int32_t>(width),
                             static_cast<int32_t>(height));
    wl_surface_commit(surface);

    LOG_DEBUG("Committed surface to compositor: %dx%d", current_buffer->width,
              current_buffer->height);
}

// ============================================================================
// PlatformWindow - Visibility Control (no-op on Wayland, background mode not
// supported)
// ============================================================================
...
{
    (void)hotkey;
    // Global hotkeys not supported on Wayland due to protocol security
    // restrictions
    return false;
}
