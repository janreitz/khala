...
int main()
{
    ...
    std::vector<indexer::DesktopApp> desktop_apps = indexer::scan_desktop_files();
    ...
    auto index_future = std::async(std::launch::async, [&]() {
        indexer::scan_filesystem_streaming(config.index_root, streaming_index, config.ignore_dirs);
    ...
    auto rank_future = std::async(std::launch::async, [&]() {
        ...
        while (!should_exit.load(std::memory_order_relaxed)) {
            ...
            if (processed_chunks >= available && !streaming_index.is_scan_complete()) {
            ...
            while (processed_chunks < streaming_index.get_available_chunks()) {
                const PackedStrings* chunk;
                if (!streaming_index.try_get_chunk(processed_chunks, chunk)) {
                    break;
                }
                ...
                for (const auto& rank : chunk_results) {
                    file_results.push_back(FileResult{
                        .path = std::string(chunk->at(rank.index)),
                        .score = rank.score
                    });
                }
                ...
                accumulated_results = merge_top_results(
                    accumulated_results, file_results, 20);
            ...
            if (streaming_index.is_scan_complete() && 
                ...
                while (ranker_mode.load(std::memory_order_acquire) == RankerMode::FileSearch &&
    ...
    while (true) {
        ...
        if (event == ui::Event::NoEvent) {
            ...
            if (std::holds_alternative<ui::FileSearch>(state.mode) && 
                ...
                const auto& result = current_file_results[state.selected_item_index];
                printf("Selected: %s\n", result.path.c_str());
                
                ui::Item item;
                item.title = result.path;
                item.command = CustomCommand{.path = fs::path(result.path), .shell_cmd = ""};
            ...
            // App search mode - search desktop applications only
                ...
                for (const auto &r : ranked) {
                    ...
                    state.items.push_back(ui::Item{
                        ...
                        .command = CustomCommand{.path = std::nullopt, .shell_cmd = app.exec_command},
                    });
                }
            } else {
                ...
                ranker_mode.store(RankerMode::FileSearch, std::memory_order_release);
            }
        }
        ...
        while (result_updates.try_pop(update)) {
            if (std::holds_alternative<ui::FileSearch>(state.mode)) {
                ...
                state.items.clear();
                state.items.reserve(std::min(current_file_results.size(), 
                                           static_cast<size_t>(config.max_visible_items)));
                
                // Display actual file paths from results
                for (size_t i = 0; i < std::min(current_file_results.size(), 
                                              static_cast<size_t>(config.max_visible_items)); ++i) {
                    const auto& result = current_file_results[i];
                    ui::Item item;
                    item.title = result.path;
                    item.description = fs::path(result.path).parent_path().string();
                    item.command = CustomCommand{.path = fs::path(result.path), .shell_cmd = ""};
    ...
    if (index_future.valid()) index_future.wait();
    if (rank_future.valid()) rank_future.wait();
